...
| **v0.7.3 changes** | **The code reader was held against a real car for the first time, and four faults were waiting — every one of which read as working equipment.** An OBDLink MX+ found nothing on a phone, and the theory that fit best was wrong: the adapter turns out to use the **standard** Serial Port Profile, so pairing it to a desktop and reading the port back settled in one command what three rounds of searching could not. What it actually needs is Chrome's **“Nearby devices” permission**, which Android 12 put in front of every Bluetooth device and which nothing reports the absence of — the chooser simply opens empty, indistinguishable from an adapter that will not pair. It is a numbered step now, on the page and in §10, rather than a paragraph somebody reaches after giving up. Talking to the adapter then exposed three decoding faults that no unit test had caught, because all three produce a confident answer. **`ATS0` turned the byte separators off** while the decoder split on whitespace to find them, so a reply parsed as one enormous number and every code vanished. **The CAN DTC count byte was decoded as code data**, shifting every code by a byte: `43 02 01 33 04 20` read as **P0201 rather than P0133 and P0420** — worse than reading nothing, being a real-looking code for a fault the car does not have, and wrong on essentially every vehicle since 2008. Payload parity now says which framing arrived, since CAN sends `1 + 2n` bytes and the older protocols `2n`. **And `UNABLE TO CONNECT` was reported as “no codes found”** — the socket is powered from the battery whether or not the car is awake, so an ignition-off read looked exactly like a clean bill of health and ended the diagnosis. The **BLE transport §8.3c always specified** also shipped, since a Bluetooth Classic adapter is unreachable over BLE and a BLE one unreachable over RFCOMM; the page had been claiming “ELM327 over USB”, which is the opposite of the truth on a phone, where Web Serial reaches Bluetooth and not USB. |
