...
| **v0.9.5 changes** | **A scan tool that connects and says nothing.** The GEARWRENCH GWSCAN reaches a working Bluetooth connection and then answers no ELM327 command at all: the Nordic UART profile it advertises is a pipe rather than a protocol, and what goes through it is the vendor's own business. Guessing at the framing cost days, and an Android `btsnoop_hci` log of the vendor's app settled it in an afternoon — `btsnoop.py` is the part of that afternoon worth keeping, a capture reader down through HCI reassembly, ATT, and the adapter's byte-stuffed framing, which is the layer a first implementation gets wrong and then blames on a flaky link. The reader built from it is **implemented and off by default** (`GWSCAN_READER`) and reached only after an ELM327 handshake meets silence, so no device is fingerprinted and a working ELM327 never meets it. A multi-frame reply is declined rather than read short, because a list of codes quietly missing some is worse than no list, and that is exactly the case the captures never contained. What answered is recorded on the session, since a reading taken over an inferred protocol is not the same claim as one from a proven tool. The codec lives in Python where it can be tested and in JavaScript where the transport has to be, with a test that runs both over the same bytes. **And silence is reported as silence**: an adapter that accepts every command and never replies used to decode to zero trouble codes, which is a clean bill of health from a device that has not spoken to the car. §8.3, C-4. **What a file is, is read from the file.** `Content-Type` on an upload is a claim by the client, and on Windows it is one the browser makes by looking the extension up in the registry — `.webp` and `.avif` frequently have no entry there, so a browser sends `application/octet-stream` for a file whose first twelve bytes say plainly what it is. Filed on that claim, a photograph became a *document*: the wrong card, and no thumbnail ever, since nothing that is not an image gets a preview — a labeled gray box nobody could look at without downloading it. A short table of signatures decides instead, content then the claim then the filename, and what cannot be identified is not guessed at. `mime` and `kind` became derived columns in the §5.4 sense, and `refile_media` applies the improved reading to what is already stored. FR-DOC-11, FR-DOC-10. **A figure that broke in half, found in eleven places.** The vehicle's costs page rendered `$392.65` as `$39`, `2.6`, `5` stacked — the detail column sized itself to a parts list and left the money column narrower than the money. `.num` was already the rule that a figure is one value; this table and ten others were not asking for it, and four of them carried the same 1.75rem inline style that is now `.figure`. The table went from three columns to two, with the detail under the label it explains; an empty vehicle gets an empty state rather than a `$0.00` total that reads as a measurement. **`1.000× Caliper` was `:g` on a `Decimal`**, which keeps the coefficient it was handed where a float would trim it; every quantity column is a Decimal, so none of the seven sites were trimming. One `format_quantity`, and the tax rate's hand-rolled copy of the same arithmetic is gone. FR-COST-2, §5.6. **Every photograph was filed as Other, and nothing could say otherwise.** Both upload routes wrote the default role and no screen could change it, so the vehicle's story printed a column of the word — the record admitting it did not know. `media_rename` became `media_describe`, taking the role with the name; photos and documents carry the form on the vehicle and the job; a role nobody set prints nothing. FR-DOC-1. **A fluid sample takes its report as it is recorded**, PDF or photographs, instead of save-then-find-the-upload-box while the PDF is open on the screen being typed from; a photographed report is a thumbnail, not `IMG_4032.jpg`. FR-DOC-10. **The schedule assumed every interval started today.** `last_done_on` and `last_done_usage` existed and `recalculate` preferred them, but the add form never asked, so a truck bought with an oil change 5,000 miles overdue was filed as due in 5,000 more. The form asks; a date given is recorded as the completion it describes, so it reaches the vehicle's history by the same route as the Done button; a future date is refused. The Done row gained a meter box, and `complete` no longer stamps today's odometer onto a service done eight months ago — left unknown, the projection is identical and the record is not false. FR-MAINT-5, FR-MAINT-6. **The schedule page had never been in the order it claimed.** `Meta.ordering` is dropped from any query that aggregates, and the page counted completions in the same query, so it showed insertion order and called it soonest-first; found when up/down buttons swapped a row with one that was not beside it on screen. One `arranged()` states the order, and the page recalculates before it fetches rather than after. Items can be arranged by hand (`sort_order`, zero everywhere until the first move); a service the list has never heard of can be named on the add form instead of through the Django admin or a whole template for one line. **Feedback landed where nobody was looking.** `liveform.js` swaps a card without moving the page, so Set on the eleventh item wrote a banner above the fold and Done cleared the date box: the confirmation is now pinned to the row, carried on the querystring because that is what survives the swap, with the words looked up from a fixed set rather than printed off the URL. **Due can act.** Done, Snooze and Ignore post from the row and come back to the list; snooze means "not now" for `SNOOZE_DAYS` — a setting, where it was a literal 30 in a view — and the button says how long. FR-MAINT-7. **Services recorded on the schedule reach the vehicle's story**, the ones with no job behind them, which is most of a home garage's history. FR-VEH-10. **A confirmed scan records its mileage on the meter** — the one reading in the whole record that arrives dated and sourced, and the one the meter never learned about; the same report confirmed twice is one visit to the odometer. FR-VEH-9, FR-INT-4. **A mistyped reading can be taken back.** Append-only forbids the edit and is right to; it left `15000` for `105000` with no remedy but a rollback-flagged correction and the wrong row for ever. The row goes to the trash, the meter falls back to the reading before it, and the audit log says who. §5.4. **Manual libraries are the shop's to add**, not the three the author knew; a library is a name and an address, nothing is ever fetched from it (NG-7), and removing one takes its pinned pages to the trash with it. The vehicle page's Browse link had never rendered for any library — the template called a method that takes the vehicle, and Django renders a swallowed `TypeError` as nothing. §8.5. **Outgoing email has a Test settings button** that sends one message with what is saved and repeats the server's own words when it fails; the two tabs that both read Backup are Backup schedule and Backups. R-9. Number boxes that clipped `10000` to `1000(` are as wide as the numbers they hold. **The Due list looked cached.** `status` is a stored projection, and stored is what the board's card, the Due list, the digest and the vehicle report read — deliberately, so a card per vehicle costs no odometer arithmetic — but nothing rewrote it except opening that vehicle's own schedule, so a changed look-ahead showed up only once every vehicle's page had been visited. It is rewritten where its inputs change: `runtime.save` refreshes the fleet when either look-ahead threshold changes, `UsageReading.save` refreshes its vehicle on every reading saved or removed (the importer, the API replay and a soft delete all reach the row without `record_reading`), and a daily `maintenance.refresh` job covers the one input no write announces. A refresh writes only the rows whose answer moved. **And its order was not random**: every Safety item outranked every routine one whatever the dates, so a registration due in 213 days sat under three safety items due in 365. Overdue safety still leads; among things merely coming up, soonest is first, sorted on the projected date because an item due by distance has no calendar date and was sorting last however soon it was. FR-MAINT-7. The schedule's arrange buttons have room around them, and the name column a floor. **Five more, from reading the code just changed.** `reading_create` was the one write route the §12.2a sweep missed — it never asked which vehicle, so a helper granted read on one could record odometer readings on any vehicle in the shop, and readings drive every distance-based due status. `1.000` survived in the templates that printed the column bare: a `quantity` filter, the project's first template tag library, on every one of them. A LubeLogger pull lands readings one row at a time, and each would have refreshed the same ten items — `refresh_later()` holds them and refreshes each vehicle once, against the meter as finally imported. The full-history page inherited the summary's caps and quietly dropped everything past the sixtieth event while saying nothing here was cut; the caps are the summary's alone now. FR-VEH-10. And a lab report photographed for a fluid sample was never OCR'd, because whether a file wants reading was decided once from its kind and role: `ingest(ocr=True)` asks outright, and a photograph re-filed as a receipt through Describe is read after the fact. FR-DOC-5, FR-COST-4. **Five gaps found by reading outward from what had just changed.** **A recall was only ever found by somebody who went looking.** Nothing checked on a timer, no vehicle recorded when it was last asked about, and an open campaign reached no screen but its own — while the module's whole posture is that an empty list must never read as a clean vehicle. The silence is fixed without adding a single call anybody did not ask for: `RECALL_CHECK_DAYS` is zero, and zero is the point (P-1, NG-7), so the only thing that asks NHTSA is the button on the vehicle's page until an operator sets an interval. With one set, `recalls.sweep` looks up exactly one vehicle an hour, oldest answer first — one, because NHTSA answers a rate-limited request with the same status and the same body it uses for “no campaigns”, so a burst would turn one rate limit into a fleet of clean bills of health. `recalls_checked_at` is stamped only by a usable answer, never by an ambiguous 400 or a refusal, and the page prints it: “nobody has looked” and “NHTSA listed nothing” were one sentence and are now two. The open campaigns already recorded reach the dashboard and the digest, which needs no network at all. §8.4, FR-MAINT-10. **Imported history was stamped as completed the moment it was imported.** `opened_at` took the record's own date and `completed_at` took `now()`, and the printed vehicle report — the document you hand a buyer — orders and dates its service history by `completed_at`. Eleven years of a LubeLogger pull printed as eleven years of work finished that afternoon, in no particular order. The CSV importer beside it had always stamped both. **And imported readings were filed under the odometer whatever the meter was**, so an hour-metered machine carried two series under two names; nothing looked wrong because `latest_reading` does not filter by meter, while the rollback check does. Both writers are fixed and `repair_imported_history` corrects what they already wrote, narrow at both ends: a job somebody genuinely finished later keeps its date, and a reading whose unit no longer matches the asset is left alone rather than filed as hours. FR-INT-13, FR-REP-2. **Brake fluid with a date on it, and a part below its minimum, never left the parts pages.** Both are dated facts of exactly the kind this digest already carries for registrations and return windows. An expired lot is urgent and one approaching its date is routine; low stock is one alert and not one per part, keyed on which parts are low so tomorrow's shortage is not silenced by today's cooldown. `restock_list` went from a query per part to one. FR-INV-4, FR-INV-6, FR-MAINT-10. **A failed read was a dead end**: the health screen counted the unreadable files and the only way to try one again was to delete it and upload it a second time, losing its caption, its role and its place in the vehicle's story — while most of these fail for reasons outside the file, like a language pack that was never installed, fixed on that same screen with nothing to act on afterwards. FR-DOC-5. |
...
| FR-VEH-14 | **The board: a vehicle list arranged by the person reading it.** The Vehicles screen answers "which of these is the truck", not "list my vehicles", and an alphabetical grid of identical cards answers the second. Three parts, all **per user** — order, color and pins are answers to a recognition question, and two people in the same shop answer it differently, so this is `asset_card_preference` and never a column on the vehicle. **Order** is stored, unplaced vehicles sort last alphabetically so adding a car never reshuffles a board somebody arranged, and rearranging inside a filtered view swaps which *slots* the visible cards hold, leaving everything hidden interleaved where it was. The same order drives the dashboard's Fleet panel, which was previously sorted by `updated_at` and therefore reshuffled itself every time anybody recorded a reading. **Color** is decoration and never encoding (§9.5): a stripe and a 7% tint, with the status pill, the due badges and the code count each carrying their own words. **Pins** choose what one card shows from a fixed catalog — year/make/model, status, photo, meter, masked VIN, plate, engine, owner, driver, what is coming due, open work orders, open trouble codes — chosen per vehicle, because a plate is meaningless on a generator and engine hours identify one where a plate identifies the other. The nickname is not a pin: it is the link's accessible name, and a card able to switch it off would be an unlabeled link. A pinned VIN is **masked** (NFR-S-5) — the board is the most-visited screen in the application and the easiest one to read over a shoulder. |
| FR-VEH-15 | **Rearranging works with no script, and dragging is an enhancement of it.** Every card carries ↑/↓ buttons posting an ordinary form, for the same reasons FR-WO-12 gave: a drag needs a script to exist at all, is unreachable from a keyboard without a second mechanism beside it, and is unkind on a phone held in one gloved hand. The gesture is then added on top for pointers that have one — pointer events rather than the HTML5 drag API, so touch is one code path with mouse and pen — and posts the finished sequence to the same board with the same filter attached. The visible list is rebuilt on the server from what that person may see, so a submitted sequence naming a vehicle that was never on the screen moves nothing. |
...
| FR-UX-1 | **A form says what its button is about to do.** Editing a purchase opened a page headed *New order* carrying a *Create* button, because the create view was written first and the edit view was pointed at the same template — so the screen claimed twice over to be making a second copy of the record already on it. Somebody who believes that either abandons the edit or saves and then goes hunting for the duplicate; either way the application has lied about the one thing a form has to be clear about. Swept rather than corrected in place: **a template that picks its Cancel target with a conditional is serving both create and edit**, which is a structural fact rather than a guess, and such a template must branch its heading and its button label as well. Five of the six already did, using a plain *Save* that is honest in both directions. |
...
| FR-PART-6 | Fitments are editable and removable, and one of the things a fitment can say is **`does_not_fit` — tried it**. Another is **`general_purpose` — fits anything**, for a part fitment is not a question about: an A/C system flush, brake cleaner, a shop rag, a hose clamp. The other four are all statements about *evidence* — somebody held it against a car, a vendor claimed it, nobody has checked — and none of them is true of a bottle of cleaner; `unverified` was the one reached for by default, and it leaves a permanent question mark against something nobody will ever answer. Choosing it sets `is_universal` rather than asking for the same fact twice, because two controls for one fact are two chances to disagree; the reverse does not hold, since a vendor may *state* that a part fits anything and that is a claim somebody may yet disprove. A vendor's claim is re-recorded every time its order is imported, so deleting a disproved one is undone by the next import; recording the failure is the shop's own knowledge and outranks the claim. An import never overwrites or resurrects a fitment that has been edited or removed. |
...
| FR-PART-8 | **A part is installed; a consumable is used up — and the catalog says which.** `is_consumable` existed from the first migration, was asked for on the form, and was then read in exactly one place: the part chooser's ranking, where it demotes a part that fits everything and is planned for nothing. It appeared on **no screen in the application**, so the only way to learn whether the shop had marked brake cleaner as a consumable was to open its edit form, and the parts list — which is where the question is actually asked — showed a case of it and a water pump as peers. The list now splits All / Parts / Consumables, each tab carrying its count **under whatever filters are already applied**, because a tab that turns out to be empty has answered the question one click too late. An unrecognized `kind` shows the catalog rather than nothing: an empty parts screen is indistinguishable from a shop that owns nothing, which is a worse lie than showing too much. The row carries the badge only where it distinguishes — on the Consumables tab every row is one, and a badge on everything is a badge nobody reads. |
| FR-PART-9 | **A part is in as many categories as it is actually in, and each one is a row.** A headlight bulb is electrical and it is lighting; a brake line is brakes and it is hydraulics, and neither of those contains the other, so a tree does not answer it either. A single field made somebody choose, and the part was then unfindable from the direction they did not choose — the exact failure the category existed to fix. It also invited the choice being dodged: with one box, `Electrical/Lighting` gets typed, and that is a third category matching neither filter. **A `Category` row, case-insensitively unique**, rather than a string on the part: the convergence this exists for cannot be a helper function every writer has to remember to call, which was the previous shape and which the CSV importer walked straight past — writing whatever the spreadsheet said, in the one place a catalog arrives hundreds of rows at a time. A constraint cannot be walked past, so `brakes` typed anywhere lands on the existing `Brakes`. Written text splits on `, ; / |`, because `Electrical/Lighting` is what somebody writes when a form has room for one answer and reading it as two is what they meant. The form is **checkboxes for what the shop already files under, plus a box for anything new** — both work with no script, and on a phone ticking boxes beats a multi-select. Nothing is rejected: an unrecognized category is a new one, which is why there is a box at all. A category is offered only while a part is in it (an empty one is a filter that finds nothing) but is never deleted, so the name returns to the same row. Every row and the part's own page name every category it is in, each a link to the rest of that group; search, category and kind compose and survive paging. Carried through every route: `GET /api/v1/parts` takes the same three filters through the same `matching()` and `KINDS`, `GET /api/v1/parts/categories` hands a client the same picker, and the importer resolves through the same rows. §7.4, §10. |
| FR-INV-15 | **The catalog is narrowed by the two questions a garage actually asks it**: *what fits the car on the lift*, and *what have I got*. Both facts are printed on every row already (FR-INV-12), which makes each of them answerable only by reading all of them — and a catalog read from memory is how a part gets ordered that is already in the drawer. **Fits this vehicle** goes through the same `fits` rule FR-PART-4 states, so it means one thing everywhere: confirmed first, a part held against that car and rejected excluded outright, and the parts that fit anything (FR-PART-6) included, because leaving brake cleaner out of what fits a vehicle is a wrong answer rather than a tidy one. The picker offers only the vehicles the reader may see, since filtering by one they may not would answer a question about it. **On the shelf / none on hand / below minimum** reads the ledger's projection, counting alive lots only — a related manager hides the trash and a join does not, and the two disagreeing would make the filter and the row's own figure tell different stories. Every narrowing composes with the search, the category and the kind split, and the kind tabs carry their counts *under the filters already applied*. A value that is not offered — an edited URL, a stale bookmark, a vehicle since made invisible — narrows nothing rather than emptying the screen, which is the posture `kind` already took. A narrowed screen says it is narrowed and offers the way back, because a filter matching nothing and a shop owning nothing look identical and only one of them has an exit. |
...
| FR-INV-13 | **A part is measured in whatever it is sold in, and a quantity may be entered in any unit of the same kind.** Four units were hard-coded — each, liters, kilograms, feet — which is a guess about somebody else's catalog: R-134a is sold in cylinders **by the pound** and dispensed **by the ounce or the half-kilogram**, and none of those were sayable. Mass, volume, length and count are all available, built from the conversion table rather than listed beside it so a unit that exists is a unit the arithmetic knows. Stock is always **held in the part's own unit** — one number per part, so the shelf total never depends on which box somebody typed into — and the conversion is quantized to the ledger's three decimal places at the edge, so what lands is what was shown. A counted part offers no unit picker: there is no factor between a gasket and a liter. |
...
| ID | Requirement |
| --- | --- |
| FR-PUR-1 | Record a purchase with lines, tax, shipping, discounts, and core charges, and attach the receipt image. **A supplier's order confirmation may be read in instead of typed**: the PDF becomes the purchase, its lines, the parts, their fitment, and — where a line is listed as a kit component — the kit it belongs inside along with its share of the kit's cost (FR-INV-9), after a preview the operator confirms. **A discount comes off before the tax is worked out, and the tax may be stated as a rate.** It was the other way round — lines taxed whole, discount subtracted at the end — which charges tax on money the shop was never charged for. The fault survived because it could not be seen: with tax stated only as an amount, `subtotal + tax - discount` and `subtotal - discount + tax` are the same number, so a model that was wrong about *what tax is charged on* produced a right-looking total. It became visible the moment a rate existed. A rate is also the more durable of the two statements — an amount is correct about one arrangement of lines and stops being correct, silently, when a line is added or corrected — so `tax_charged_minor` is **derived** from the rate rather than written back on save, because what makes it stale is a line changing and lines are edited from another screen. An amount is still accepted and is what an imported order confirmation carries (§8.3a), a rate wins where both are present, and shipping is outside the taxable base because whether a carrier's charge is taxable is a question about a jurisdiction rather than about this order. The totals panel prints the taxable figure and the rate that produced the tax, so the arithmetic can be checked against the receipt in somebody's hand. |
| FR-PUR-11 | **A purchase line stores what it cost, not what one of them cost.** The form asked for a price *each*, which is frequently a number the vendor never stated: five gallons of brake cleaner is sold as $182.39 for the five, so the figure had to be divided out by hand — $36.478 — and the column it went into holds whole cents, so it became $36.48 and the line then claimed $182.40. A cent appeared that nobody paid and the order stopped agreeing with the receipt. **The error is a category one rather than a rounding one.** Money is an integer number of minor units because that is what survives arithmetic (§5.5) — but a unit price is not an amount anybody paid, it is a **rate**, and rates do not divide evenly. The same distinction this section's tax now makes: `tax_rate` is a rate and is a `Decimal`, `tax_charged_minor` is money and is cents. So `extended_minor` is stored exactly and the per-unit figure is derived from it — `unit_price_exact` for every calculation that spends the line's money, rounded once at the point money is actually written. The add-line form accepts **either** box because a receipt states whichever the vendor felt like printing, and the total wins where both are given, being the figure that was charged rather than arithmetic somebody did on the way to the box. The row prints both, because $36.48 beside five gallons and a $182.39 total is a row that does not add up and leaves the reader to guess which number lied. Receiving also **rounds rather than truncates**: `int()` on a Decimal threw away a share of tax and shipping that is always positive, so every lot ever received landed slightly cheaper than it was, in the same direction every time. |
| FR-PUR-10 | **A supplier order document is recognized, not declared, and a general retailer is not a parts vendor.** RockAuto was the only reader, so the shape it returned lived inside it and the import service named RockAuto in six places; two more vendors make that the wrong arrangement, because what differs between suppliers is the *document* and nothing else. The shape is `orders.py`, the vendor identifies itself on the order it returns, and the file is offered to each reader in turn — a reader recognizes its own format or **raises**, because one that tried hardest to extract something would be wrong in the expensive direction. **NAPA** prints the extended figure per line and a list price beside it, so it is taken directly (FR-PUR-9); its summary is also the evidence for the discount-before-tax rule, since 8.4% of its 28.05 subtotal is 9.16 and it is 8.4% of 23.05 that gives the 8.74 it printed. It states a pickup date, which is what a return window runs from (FR-PUR-5). **Amazon** is not a parts vendor at all: one sample basket holds eight items of which exactly one is a part and seven are tools, and nothing in the document says which — so the reader takes no view and **the review screen asks**, one choice per line: a part, the tooling it was, or nothing. **Tooling here is a spend and never an inventory** — NG-8 and NG-9 put tool and toolbox tracking permanently out of scope and WrenchLedger is where it lives, while OQ-4 keeps `expense.category = tooling` first-class, always exported and excluded from per-vehicle cost. So such a line becomes one `expense` row carrying the amount, the vendor and the date, with **no part, no `ShopTool`, no location, no serial and no model**, and with `asset` null rather than merely excluded by a setting: an expense attached to no vehicle cannot be pulled into one by a later change of mind about `COST_INCLUDE_TOOLING`. Whatever a line becomes it takes its share of tax and shipping with it, allocated by the same largest-remainder split a kit's cost uses, so the order still adds up to what it cost and nothing is banked twice. What is left out is left out of the money: tax and shipping are apportioned to the lines kept, because a purchase claiming 2.72 of tax on 4.24 of relays is a cost history that reads plausibly and is wrong for ever (G-4). Amazon prints a price *each* where NAPA prints a total, which nothing but **reconciling against the document's own figures** revealed — read the other way, a two-parcel order of washer fluid reported 1.94 of 5.82. A marketplace seller is recorded as the seller and never as the brand. §8.3a. |
| FR-PUR-12 | **A line is however many of the part arrived, which is not always how the vendor counted.** Amazon sold a two-pack of automotive relays as `1 of: 2Pcs 156700-2480 ... Relay` for $14.24 — one line item, one charge and **two relays**. Read straight in, the shelf gained one relay costing $14.24 and the second one did not exist. `qty_ordered` has meant *how many of the part* everywhere else since a line started holding its extended price (FR-PUR-11): receiving puts this many on the shelf, readiness counts this many as on order, and the add-line form has asked for it in those terms all along — which is why typing that purchase by hand worked and reading it in did not. Only the importer disagreed, by copying the vendor's line count straight across. So the review screen asks per line, beside the choice of what the line *is*, and **the count is the only thing it asks**: `extended_minor` stays the document's own figure and the per-unit price is derived from it, so a corrected count changes what the shop has and never what the order cost — the order still reconciles against its own printed total afterwards. **The count is asked for rather than read.** A product title often says `2Pcs`, `6-Pack` or `Case of 4`, and across three sample orders a pattern that turned those into numbers found one real pack, called a `1-Pack` a multipack, missed `1 Gallon (Case of 4)`, and read `6-Pack` off a title for one kit. A count nobody stated doubles what the shelf claims to hold and halves what it claims to have cost, which is a cost history that reads plausibly and is wrong for ever (G-4). So the phrase is **quoted verbatim beside the box** in the seller's own words and the operator types the number — the same division of labor the rest of that screen makes, where the reader states what the document says and the person decides what it means. A count that is not a number, is zero or negative, or is larger than any real order is dropped in favor of the document's own, and the screen says so. |
...
| FR-DOC-11 | **What a file is, is read from the file.** `Content-Type` on a multipart upload is a claim by the client, and on Windows it is one the browser makes by looking the extension up in the registry — `.webp` and `.avif` frequently have no entry there, so a browser sends `application/octet-stream` for a file whose first twelve bytes say plainly what it is. Filed on that claim, a photograph became a *document*: the wrong card (FR-DOC-10), and no thumbnail ever, since nothing that is not an image gets a preview — a labeled gray box that could not be looked at without downloading it. The bytes decide instead, from a short table of signatures covering the formats this application does something specific about; **content, then the claim, then the filename**, because a claim is evidence and a name is a hint and neither outranks the file. What cannot be identified is not guessed at — the claim stands, and an unrecognizable file is still stored and still downloadable. `mime` and `kind` are therefore **derived** columns in the §5.4 sense rather than client-written ones: the bytes never change, only the reading of them improves, and `refile_media` applies that improvement to what is already stored. What a file is *for* — a scan-tool export, an audio note — is not re-decided by this, because no amount of reading the bytes answers that question. |
...
| FR-ADM-6 | Import from CSV with column mapping for vehicles, parts, and service history — **so the spreadsheet this replaces can actually come along**. **A mapped column is read or it is not offered.** The parts schema advertised `unit`, `quantity`, `location` and `cost` and the service schema advertised `cost` and `vendor`; the operator mapped all six on the import screen and **all six were discarded**. A spreadsheet that knew the shop owned nine oil filters in Cabinet B produced nine filters' worth of nothing, and a file of eleven years of receipts produced eleven years of work orders that had all cost zero — which is G-4, the whole reason to keep the history, reading wrong and looking complete. Stock now arrives the only way stock ever arrives (FR-INV-1): a lot and a `receive` line in the ledger, never a number written into `qty_on_hand`. A service cost becomes an `Expense` against the work order and the vehicle, under `outsourced_labor` — the honest category for a row that says only what it cost — and a named shop is matched before it is made, so importing two files does not leave two of it. A cost that cannot be read, a unit nobody measures in, and a yes/no cell that is neither are each **reported and the row skipped**, rather than silently defaulted: unlike a missing year, a wrong unit or a `False` leaves nothing to notice afterwards. Everything that can be refused is refused before the dry-run return, so a preview turns down exactly the rows the real run would. |
...
**Web Serial** (USB) or **Web Bluetooth** (BLE) — the *browser* talks to the adapter and posts results to the server. This is deliberate: the app runs in a container with no access to host USB or Bluetooth, and the phone in the garage is the device actually near the car. Requires a secure context (C-1) and a Chromium browser (C-4); the UI must state the requirement rather than failing mysteriously. **Both transports are required, not alternatives**: a Bluetooth Classic adapter is unreachable over BLE and a BLE adapter is unreachable over RFCOMM, so shipping one strands the other's owners. **A paired adapter is not necessarily a visible one.** Web Serial offers an unmapped Bluetooth port only when it carries the standard SerialPort service class or a UUID the page has named; desktops mask this by mapping paired adapters to a serial port first, so the failure appears only on the phone, which is the only device that was ever going to be in the garage. The allow-list is therefore configuration, and the page must print the service class ID it connected to — no vendor documents it, and connecting once on a desktop is the only way to find it. **An adapter that answers is not a car that answered**: the socket is powered from the battery, so a link error must be reported as a failed read and never as an absence of codes. Read-only operation: read DTCs, freeze frame, readiness, VIN (mode 09), and live PIDs for a snapshot. **Clearing codes is supported but requires a confirmation that names the vehicle and warns that readiness monitors will reset** — clearing before an emissions test is a genuine, expensive mistake.

**An adapter that connects and says nothing is not a broken adapter.** A tool
speaking its maker's own protocol accepts the connection, accepts every command
and never replies, which decodes to zero trouble codes — a clean bill of health
from a device that has not spoken to the car. Silence is therefore reported as
silence, and the same holds for clearing. One such protocol is **implemented
and off by default** (`GWSCAN_READER`): the GEARWRENCH GWSCAN, a rebadged XTOOL
whose framing was read out of captures of its own app. Three things make that
defensible rather than reckless. It is reached **only after** an ELM327
handshake meets silence, so no device is fingerprinted and a working ELM327
never meets it. A **multi-frame reply is declined rather than read short**,
because a list of codes quietly missing some is worse than no list — and that
is the case the captures never contained, which is why the default is off.
And **what answered is recorded on the session**, since a reading taken over an
inferred protocol is not the same claim as one from a proven tool. The codec
lives in Python where it can be tested and in JavaScript where the transport
has to be, with a test that runs both on the same bytes wherever a JavaScript
runtime exists.
...
3. **Nothing asks NHTSA on a schedule unless the operator sets one.** `RECALL_CHECK_DAYS` defaults to zero, and the default is the point: a shop that has chosen to stay offline (P-1, NG-7) must not discover that its fleet is being announced to a federal API every week because a default said so. Until an interval is set, the only thing that reaches NHTSA is the Check button on a vehicle's own page.
4. With an interval set, `recalls.sweep` runs hourly and looks up **exactly one vehicle**, the one whose answer is oldest. One, not a batch: NHTSA answers a rate-limited request with the same status and the same body it uses for “this vehicle has no campaigns”, so a burst of twelve would turn one rate limit into eleven clean bills of health — the one failure this feature must not have. The sweep skips vehicles `check` cannot look up at all (no year, make or model; a region NHTSA does not cover; equipment, FR-EQP-3), since one of those taken repeatedly would starve the rest of the fleet. Offline Mode stops it like everything else.
5. **`recalls_checked_at` is stamped only by a usable answer.** Not by the ambiguous empty 400, not by a refusal, not by an unreachable service. The vehicle's page prints it, because an empty campaign list on a safety screen reads as “this vehicle is clear” unless the page says when it last looked — “nobody has asked” and “NHTSA listed nothing” are different claims and are shown as different sentences.
6. Open campaigns already recorded surface on the dashboard and in the reminder digest (FR-MAINT-10). That path needs no network at all: it is reading rows this shop already holds, and it works with recall checking switched off entirely.
...
**A field says what it is.** Not what it is *called* — the label does that, and
a label is a name rather than an explanation. Every field carries a sentence
about what it is or does, which the browser offers as a tooltip and a screen
reader as the field's description. It is `title`, not help text under the box:
a paragraph beneath all one hundred and ninety fields would bury the handful
that are genuinely surprising, and a description is worth reading when you go
looking for it and noise when it arrives unasked. The sentence has a second
job — *"Please fill out this field"* is what a browser says about a required
control that describes itself to nobody, and it is true of every empty box and
useful about none, so `forms.js` swaps the field's own description in. **Only
the missing-value message is replaced**: out of range, wrong format and too
long already name the actual problem, and a general description there would
take away the one useful sentence rather than add one. Nothing in that path
validates anything; which fields are required is stated in the markup and
enforced on the server.

**Figures are one value.** A quantity broken across two lines after `1.00`, or
a price after `$79.`, renders perfectly and reads as a number nobody typed —
and nothing on the screen says which of the two readings is right. Columns of
figures opt out of the breaking that a VIN and a part number need, and where
that makes a table wider than its card the table scrolls rather than the
numbers being crushed.

**An identifier is not a figure**, and holding one on a line has the opposite
cost. `BECK/ARNLEY 0832688 (083-2688)` unbroken takes most of an order review
card, and a vendor that prints its grade in the same field takes more than the
card has — so the identifier column breaks and the figures beside it do not.
Where a table is wider than its card the column that should give way is the one
that *absorbs* the slack rather than one that demands a floor, and each column
says which it is **by name**: a rule addressed to `nth-child` is aimed at a
different column the moment an optional one is not rendered.
...
- **Vehicles — the board.** A list of cards the reader arranges: their own order, an optional color, and per-vehicle pins choosing what each card says (FR-VEH-14). Rearranged with ↑/↓ buttons that need no script, or by dragging where a pointer allows it (FR-VEH-15). The dashboard's Fleet panel is the same board, showing its first few.
...
- **Page caching is network-first, not stale-while-revalidate.** SWR was specified and built, and it is the wrong policy for a system of record: every write here is post-redirect-get, so the page that lands immediately after adding a part is exactly the page SWR answers from a copy taken before that part existed. Reported as *the new item is not shown until I refresh*, which reads as the application having discarded the entry. The cache is now what answers when there is no network rather than what answers first, offline behavior (P-7) is unchanged, and a **write that lands drops the page cache**, since the list it just changed is sitting in there without the change. Static assets stay cache-first: they carry the asset version in the cache name, so they cannot go stale without the name changing.
...
GET    /parts?q=&category=&kind=     kind=part|consumable; q is name|number|crossref|UPC
GET    /parts/{id}
GET    /parts/categories             what this shop files under, for a picker
...
| ELM327 direct read | **Both transports** — Web Serial for wired and Bluetooth Classic adapters, Web Bluetooth for BLE ones — because neither reaches the other's hardware and a single one always strands somebody. Modes 03/07/0A, DTC decoding from the raw bytes, and clearing behind a confirmation that names the vehicle and the readiness-monitor consequence. **Four faults survived until an OBDLink MX+ was held against a running car, and every one of them read as working equipment.** *The chooser was empty on a phone*: an unmapped Bluetooth port is offered only for the standard SerialPort service class or a UUID the page names, and a desktop hides this because the OS maps a paired adapter to a serial port and mapped ports are always offered — so the adapter that works on a laptop is invisible on the phone actually standing next to the car. `ELM327_BLUETOOTH_SERVICE_UUIDS` names them, and the page prints the service class ID of whatever it opened, because that identifier appears in no vendor's documentation and connecting once on a desktop is the only way to learn it. *`ATS0` turned the byte separators off* while the decoder split on whitespace to find them, so a reply parsed as one enormous number and every code vanished — the request is now `ATS1`, and the decoder re-pairs an unspaced run anyway, since an adapter left in `ATS0` by another app must not read as a healthy car. *The DTC count byte was decoded as code data*: CAN answers `43 <count> <hi lo> …` and the older protocols answer `43 <hi lo> …`, so counted payloads are odd and uncounted ones even, and parity decides which. Pairing from the count shifted every code by a byte — `43 02 01 33 04 20` read as **P0201 instead of P0133 and P0420** — which is worse than reading nothing, being a real-looking code for a fault the car does not have. *And `UNABLE TO CONNECT` was reported as "no codes found"*, the single worst answer this screen could give: the socket is powered from the battery whether or not the car is awake, so an ignition-off read looked exactly like a clean bill of health and ended the diagnosis. Link errors are now named as failures; `NO DATA` is deliberately not one of them, being the ordinary reply for pending and permanent codes on plenty of cars. |
...
| FR-VEH-14, FR-VEH-15 | **The board.** The Vehicles screen and the dashboard's Fleet panel arranged by the person reading them — their own order, an optional color, and per-vehicle pins choosing what each card says. Per user, in `asset_card_preference`, because "which of these is the truck" is a recognition question two people in one household answer differently. Reordering needs no script; dragging is layered over the buttons that do the work, and the geometry is frozen for the length of the gesture because measuring cards that are being moved is a feedback loop. A pinned VIN is masked — the board is the most-visited screen here and the easiest one to read over a shoulder (NFR-S-5). | §9.3 |
| FR-PART-8, FR-PART-9 | **The two facts the catalog was already storing and could not show.** Parts split from consumables on the list, with a count on each tab; the category made visible, filterable and self-converging — offered on the form from what has been used, and snapped to the existing spelling when it is typed out by hand. Both were write-only fields: `is_consumable` reached one line of chooser ranking and no screen, and `category` reached the search box's `OR` clause and no screen. A field nobody can see is a field nobody can keep tidy, which is why the filter and the suggestions had to ship together. Carried to **every route in and out**, because a field that converges on one of them and not the others does not converge: `GET /api/v1/parts` takes the same three filters through the same `matching()` and `KINDS` — a second implementation of *what counts as a consumable* is how a screen and an API come to disagree while each looks right alone — `GET /api/v1/parts/categories` hands a client the same picker, and the CSV importer both accepts `consumable` and files its category through the same canonicalization instead of writing whatever the file said. §7.4, §10. | §7.4 |
| FR-PUR-1, FR-PUR-11 | **The money on a purchase, twice over.** A discount now comes off before the tax is worked out, and tax may be stated as a rate rather than only as an amount — which is the statement that stays true when a line changes. A line stores what it cost rather than what one of them cost, so a five-gallon drum sold for $182.39 stays $182.39 and the per-unit figure is derived. Both were category errors rather than rounding ones: a rate is not an amount anybody paid. Receiving also rounds where it used to truncate, so a lot no longer lands a little cheaper than it was, in the same direction every time. | §7.5 |
| FR-PUR-10 | **NAPA and Amazon order documents read in**, chosen by recognition rather than by asking the operator to classify a file they are holding. The shape moved out of `rockauto.py` and the vendor identifies itself on the order it returns. A general retailer's basket is mostly not parts, so the review screen asks per line and can record one as tooling — a spend, never an inventory (NG-8, OQ-4). | §8.3a |
| FR-PUR-12 | **A count that is the shop's and not the vendor's.** An imported line is asked how many of the part actually arrived — a two-pack of relays is one line, one charge and two relays — and the answer moves no money, because the charge is the one thing an invoice is never ambiguous about. What the seller wrote about the pack is quoted beside the box rather than parsed into a number. | §7.5 |
| FR-ADM-6 | **The six spreadsheet columns the importer collected and threw away**, plus a `consumable` column it never had. Stock arrives as a lot and a ledger entry, never a number written into an on-hand figure. | §8.1 |
| FR-UX-1 | **A form says what its button will do.** Editing a purchase opened a page headed *New order* with a *Create* button on it. Swept rather than fixed in place. | §9.2 |
...
| The restore path is exercised in CI against the prior release's backup format | §13.2 | **CI exists now, and this is the one part of the gate it does not cover.** `.github/workflows/ci.yml` builds the image and runs the suite *inside it*, with the checkout mounted over `/app` — the only arrangement that tests the dependency set that actually ships rather than whatever the runner happens to have installed. The four gates run there (`check_translations`, `check_accessibility`, `check_rtl`, `build_catalog --check`), so §5.6's and §8.1b's half of this claim is now true and has left this table. Arranging it that way found that **the in-container suite had never been green**: `tests_locales` reads the `.po` sources `.dockerignore` deliberately keeps out of the image, so it had failed on every image built since that rule was written, and nothing ran it often enough to notice. The restore round-trip remains uncovered, and now **deliberately**: the only fixture is `Artifacts/samples/backups/`, gitignored because it is a real dump of a real shop, and CI cannot reach what git does not carry. Covering it means committing a redacted backup — the problem `capture.py` already solved for scan reports — and that is accepted work nobody has needed badly enough to do. |
...
inclusion and labor rate · reminder enablement and cooldown · house-placement
...
| Worker | It compares `config_generation` on each poll and exits cleanly when it differs; Compose's `restart: unless-stopped` brings it back within seconds. No cross-container signaling to build. |
...
| --- | --- | --- | --- |
| Pages and API reads are cached **stale-while-revalidate** | §5.4 | Exactly as specified, and specified wrongly. SWR answers from the cache and refreshes behind the reader, which is right for content nobody in the room is editing. Every write here is post-redirect-get, so the page that lands immediately after adding a part is the page SWR answers from a copy taken before that part existed. Reported as *the new item is not shown until I refresh* — which for a record-keeping application reads as having discarded the entry. | Network-first for pages, cache as the fallback; a write that lands drops the page cache. §5.4 |
| A purchase records tax, shipping and a discount | FR-PUR-1 | It recorded all three and was wrong about what tax is charged on: the discount was subtracted **after** tax. Invisible for as long as tax could only be stated as an amount, because addition is commutative and the total came out right anyway — the model was wrong and the arithmetic concealed it. A vendor's own document settled it: NAPA prints $228.05 of lines, $18.74 of tax and a $5.00 discount against a $241.79 total, and 8.4% of $228.05 is $19.16. | FR-PUR-1, `Purchase.taxable_minor`, an optional `tax_rate` |
| A purchase line records what was paid for it | FR-PUR-1, §5.5 | It recorded the price of **one**, which cannot represent a real purchase. Five gallons of brake cleaner sold as $182.39 is $36.478 a gallon — not a number of cents — so the stored figure became $36.48 and the line claimed $182.40. §5.5 is right that money is an integer number of minor units; the error was filing a **rate** as money. | FR-PUR-11, `extended_minor` with the per-unit figure derived |
| A purchase line records how many were bought | FR-PUR-1 | It recorded how many the **vendor counted**, which is a different number whenever one line item is several of a thing. Amazon's `1 of: 2Pcs ... Automotive Relay` put one relay on the shelf at $14.24 and left the second one in the drawer and in no record anywhere. The field already meant *how many of the part* to receiving, to readiness and to the add-line form; the order importer was the only thing reading it the vendor's way, and nothing contradicted it because a parts supplier's line is one of something. | FR-PUR-12, a count asked for on the review screen |
| The CSV importer maps `unit`, `quantity`, `location` and `cost` for parts, and `cost` and `vendor` for service history | FR-ADM-6 | The operator mapped all six on the import screen and **all six were discarded**. A spreadsheet that knew the shop owned nine oil filters in Cabinet B produced nine filters' worth of nothing, and a file of eleven years of receipts produced work orders that had all cost zero — G-4 reading wrong and looking complete. | FR-ADM-6 |
| A part has a category | §6.2, §7.4 | Asked for on every part, indexed twice, matched by the search box, and **rendered on no screen in the application**. It filed nothing: nobody could see what a part was under, so nobody could see that the last one went in under a different spelling. `is_consumable` was the same shape — written on the form, read only to rank a picker. | FR-PART-8, FR-PART-9 |
| Jobs are retried with backoff, and a permanent failure lands in `failed` where admin health can see it | §5.2, NFR-R-2 | True of a job that *raised*, and nothing at all covered a job whose worker died holding it — a redeploy, an OOM, the host going down. `claim` stamped `locked_at` and **nothing ever read it**: the row stayed `running` for ever, was never retried and never failed. Not an untidy row either, because `schedule.tick` skips a type that already has one in flight — so a single orphaned `backup.run` silently ended every future backup, while the backups screen disabled “Back up now” for the same reason and showed a “running” pill. Found on a live instance eighteen hours and one missed backup later. | `jobs.reclaim`, run at the head of every drain; the backups screen names a stalled job and offers to give up on it |
| The service worker caches pages and API reads, serves an offline landing, and drains the write queue on reconnect | §5.4, §9.4, P-2 | Every line of it written and none of it running. `offline.js` registered the worker at `/static/sw.js`, and a worker's scope is the directory it is served from — so it controlled `/static/` and nothing else: it installed, reported active, and never saw a page. `core.views.service_worker` existed precisely to serve it from the root with `Service-Worker-Allowed`, carried a docstring explaining this exact trap, and had two tests — which proved the route was *served* correctly and never that anything asked for it. The one line that would have used it was never changed. | `offline.js` registers `/sw.js`; `tests_phase4` now asserts the page uses the route, which is the assertion whose absence let this ship |
| A released change to the stylesheet or to a script reaches the browser | §5.4, §9.4 | Only ever for a browser that had never opened the app. `/static/` is cache-first against `VERSION`, and `VERSION` was the literal `"v1"` — while `activate` deletes only caches whose name lacks the current version. So `shell-v1` was written on a browser's first ever visit and served for the life of the installation, and every later release of `app.css`, `forms.js` and the rest was fetched into a cache nothing read. Invisible from a developer's machine and from a private window, and indistinguishable from a deploy that had not happened. | `service_worker` stamps a fingerprint of the assets into `VERSION`, so the worker differs exactly when what it caches differs; the unstamped fallback moved off `"v1"` and a misscoped worker retires itself, since the corrected registration lived in a file the stale cache was serving |
...
| The corpus is redacted captures, and nothing identifying reaches the repository | NFR-S-5, `capture.py` | True for PDFs and switched off for photographs. Every rule that protects a labeled value asks what is printed *beside* it, and `_same_line` compared word **tops**: on a real OCR capture `SN:` and its serial are 24 pixels apart, because the colon runs a full cap height and the serial is digits, against a tolerance of 21. So no label preceded the serial, so a real tester serial was written into this repository and committed. Found by grepping the new captures for the string — which is worth doing every time, and which then found the same serial in the *test* written to prove the redaction works. A value that belongs nowhere belongs nowhere, including in the test about removing it. | `_same_line` compares middles; `RedactingABenchTesterTests` |
...
| --- | --- |
| 0.9.5 | **The GEARWRENCH GWSCAN can be read**, over a protocol recovered from Bluetooth captures of its own app — implemented, off by default, and reached only when an ELM327 handshake meets silence; an adapter that says nothing now reports silence instead of a clean bill of health. **An uploaded file's type is read from its bytes** rather than from what the browser claimed, so a photograph is no longer filed as a document that never gets a preview. **The vehicle's costs page stopped breaking figures in half, and the same fault was found in ten more places**; quantities read `1×` rather than `1.000×`. Photographs can be described — name and role — where every one had been filed as Other. A fluid sample takes its report as it is recorded. **The maintenance schedule asks when a service was last done**, records that as the completion it is, and no longer stamps today's odometer on back-filled work; the page had never actually been soonest-first, because `Meta.ordering` is dropped from aggregating queries — one `arranged()` now says the order, items can be arranged by hand, and a new service can be named on the form. Confirmations sit on the row that asked. **Due can act** — Done, Snooze, Ignore — and snooze is a setting. **The Due list is current and reads soonest-first**: the stored status is rewritten where its inputs change — the look-ahead settings, every meter reading, and a daily job — rather than waiting for a vehicle's schedule to be opened, and overdue safety leads while everything else sorts on when it is expected. Schedule work reaches the vehicle's history; a confirmed scan records its mileage; a mistyped meter reading can be taken back to the trash. **Manual libraries are the shop's to add**, and the Browse link that had never rendered does. Outgoing email has a test button; the two Backup tabs are told apart. **Then five from reading the code just changed**: the one write route the helper sweep missed, `1.000` in the templates that printed a quantity bare, an import that refreshed the same items once per reading, a full-history page that stopped at sixty events, and a photographed lab report that was never read. Recalls are checked on a schedule only if you set one, one vehicle an hour; imported history keeps its own completion dates and meter; the shelf reaches the reminder digest; and an unreadable file can be read again. |
| 0.9.4 | **A general retailer's order stopped making a new part for every line.** It states no part number, so the matcher had nothing to look up: the same item on two shipment lines of one order became two catalog rows with the stock split between them. A numberless line now matches on an exact whole title, and nothing is fuzzy — a near-match would fold two sizes of one product together. The unit selector added in 0.9.2 had also pushed the status column off the side of the review card. The test suite stopped reading the operator's `PASSWORD_POLICY`, which could quietly disable the permission tests. Receiving a line no longer reloads the purchase page to the top, correcting a receipt no longer leaves an empty lot behind, a fitment can be marked Universal instead of having it typed into the make field, a part carries a supplier link, a use recorded without a job can have its vehicle and date corrected, and a core deposit can be marked **kept** rather than only returned — the reported case being a caliper whose return postage came to as much as the deposit. A part can also carry photographs, which every other record already could. **Every field now says what it is** — a sentence on hover, and the same sentence in place of "Please fill out this field" when a required one is left empty. A fitment gained a **General purpose** confidence for the parts fitment is not a question about: brake cleaner, a rag, a hose clamp. And the stock lots card stopped breaking its own numbers across two lines. FR-INV-5, FR-INV-7, FR-INV-10, FR-PART-2, FR-PART-3, FR-PART-6, FR-PUR-1, §9.2, §12.2. |
| 0.9.3 | **Every write was happening twice, and had been since 0.9.0.** The service worker fetched each non-GET itself and then returned without responding, so the browser sent it again — invisible on any idempotent write, and a duplicate of everything else. One added vehicle arrived as two, and two racing order imports collided on the provenance row's unique constraint. **And the password floor became a setting**: `PASSWORD_POLICY` chooses between `12chars` (the default, unchanged), `complex`, `6chars`, `any` and `noauth`, because twelve characters per sign-in on a machine nobody else can reach is a toll rather than a defense. An unrecognized value refuses to boot. FR-ADM-1, FR-ADM-8, §12.2. |
| 0.9.2 | **The size on the container is read, and it is not the same thing as a count.** A five-gallon pail of brake cleaner arrived as one of something, so the shelf could say how many drums were bought and never how much cleaner was left. A size stated with a unit on it — `5 gal`, `8 Ounce`, `1 Quart` — now sets the count and the part's unit, while `2Pcs` and `Case of 4` stay quoted and uncounted exactly as before: one is a regulated statement about the container, the other is marketing copy. No money moves either way. The review screen shows what was read beside a unit selector, and a part already filed under its own unit keeps it. A provenance row left behind by a deleted purchase also stopped being able to fail an import with a duplicate-key error. FR-PUR-1, FR-PUR-11, FR-PUR-12, §6.2. |
| 0.9.1 | **A delete that deleted nothing.** An order deleted from the admin left its lines alive underneath it — a soft delete runs no SQL DELETE, so `CASCADE` never fires — and the importer, resolving the leftover `ExternalRef` through the trash, refused to read that document ever again on the strength of received lines no screen could show. Soft delete now cascades where a parent declares it, and the reader agrees with the rest of the application about what has been deleted. The trash gained the half it never had: nothing had ever enforced the thirty days it promises, so `manage.py purge_trash` and a **Delete permanently** button now do, and `manage.py purge_order` clears up after a bad import without quietly taking received stock that has since moved. Nineteen models had no admin page at all; every table is registered now, the soft-deleting ones behind a record-state filter, with one Trash page spanning all of them. FR-ADM-7, FR-PUR-1, §5.4. |
| 0.9.0 | **The parts catalog started using what it already stored, and a receipt found two ways the money was wrong.** Consumables split from parts on the list; categories became rows a part can have several of, unique by a database constraint rather than by a helper the CSV importer had been bypassing; parts reached the API. The importer stopped throwing away six columns it asked the operator to map. Then a NAPA order refused to match the screen: **the discount was being applied after tax**, and **a line stored the price of one**, which cannot represent five gallons sold for $182.39. Both were category errors — a rate is not an amount anybody paid — and both are fixed at the model. NAPA and Amazon orders can now be read in, recognized by what is in them; an Amazon basket is mostly not parts, so the review screen asks per line and records tooling as a spend, never an inventory (NG-8). An imported line is asked how many of the part actually arrived, because a two-pack of relays is one line item and two relays, and only the importer had been reading that field the vendor's way. Pages stopped being served from cache before the reader's own edit. FR-PART-8, FR-PART-9, FR-PUR-1, FR-PUR-10, FR-PUR-11, FR-PUR-12, FR-ADM-6, FR-UX-1, §19. |
| 0.8.0 | **The Vehicles screen became a board somebody arranges** — per-user order, color and pinned facts, with the dashboard's Fleet panel as the same board rather than a list that re-sorted itself on every odometer reading. Reordering works with no script; dragging is layered on top, and had to be written twice because reordering the DOM mid-gesture is a feedback loop rather than a rough edge. Trying to see that fix found the bug underneath it: **the service worker had been registered at `/static/sw.js`**, so it cached a directory of stylesheets and never a page — and cached them against a constant version, which made every released change to the stylesheet and to every script invisible to anybody who had used the app before. And the instance itself reported a third: **a backup job stuck `running` for eighteen hours had silently ended every future backup**, because nothing ever read the `locked_at` a worker stamps when it takes a job. FR-VEH-14, FR-VEH-15, §19. |
| 0.7.3 | **The code reader met a real car.** Four faults, all of which looked like working equipment: an empty adapter chooser that was Chrome missing Android's “Nearby devices” permission, `ATS0` removing the byte separators the decoder needed, the CAN DTC count byte being decoded as code data — reading P0201 where the car said P0133 and P0420 — and `UNABLE TO CONNECT` reported as “no codes found”. The BLE transport §8.3c always specified now ships beside Web Serial. |
| 0.7.2 | **Compose pulls the published images**, with `docker-compose.build.yml` for anyone who needs to build. A fresh-machine install found a gap per step: §1 is now three complete platform paths, the host ports are configurable, and §7 covers restoring a backup taken elsewhere — which the UI could not do at all, since the two files it hands out are exactly the two `restore` refuses. Backup upload rebuilds the folder they came from. Instance health names the machine and lists the queue rather than counting it. |
| 0.7.1 | **Published to Docker Hub** — both images, amd64 and arm64 built natively as one manifest each. The version became a file rather than a sentence in this document; `.dockerignore` stopped shipping 349 MB of untracked research and five photographs of our own vehicles; and CI now runs the suite inside the image it publishes, which is what found that the in-container suite had never been green. §15.2. |
