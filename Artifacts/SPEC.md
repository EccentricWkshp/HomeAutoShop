...
| **v0.9.2 changes** | **A pail is a package and gallons are what the shop sprays.** NAPA charges $182.39 for `CRC Brakleen ... 5 gal (US)` at `Qty: 1`, and FR-PUR-12 — which had just finished establishing that a line's count is *how many of the part* rather than the vendor's line count — read that as one of something, because the only counting rule it had was about packs of relays. So the shelf could say the shop owned one drum and never how much cleaner was left, which is the single thing stocking a consumable is for. A size stated **with a unit of measure on it** is now read off the description and fills in both the count and the part's unit: five gallons of a part measured in gallons, at $36.478 a gallon. **The rule about bare counts did not bend to make room for it**, and the distinction is the load-bearing part. `2Pcs`, `Case of 4` and `3-Pack` are still quoted and never counted, because a number in a product title may be the pack, the pin count or the vehicles it fits — but `5 gal` is a regulated statement about the container, and nothing writes it on a pail that holds four. The money is untouched either way: `extended_minor` is still the document's own figure and the per-unit price is still derived from it, so the order reconciles against its printed total exactly as before ($228.05, unchanged). What was read is shown on the review screen beside a unit selector, because a reader that infers proposes and never decides — and a part the shop already files under its own unit keeps it, the count falling back to the vendor's rather than multiplying gallons into something measured in `each`. **And a provenance row stopped being able to fail an import.** `ExternalRef` has no foreign key, so nothing cascades to it and hard-deleting a purchase strands one; writing the new row with `create()` guarded by a lookup turned any disagreement between the two into `duplicate key value violates unique constraint` on somebody's import screen. It is upserted on its natural key now, which is also simply what it means — the row states which purchase this document is currently represented by. FR-PUR-1, FR-PUR-11, FR-PUR-12, FR-INV-5, §6.2. |
