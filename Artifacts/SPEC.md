...
| **v0.9.3 changes** | **Every write in the application was happening twice, and had been since 0.9.0.** The service worker's non-GET branch ran its own `fetch(request.clone())` and then returned — and a `fetch` handler that returns without calling `respondWith` hands the request back to the browser, which performs it itself. Two POSTs on the wire for every write: the worker's, whose response was read only to decide whether to drop the page cache and was otherwise discarded, and the browser's, which is the one the person saw. The clone is the tell — nothing needs a second copy of a body it is not sending twice. **It is invisible on every idempotent write**, which is why it shipped and survived a release: an edit setting the same fields twice looks perfect, and only a create doubles. Adding one vehicle added two of it, and two of the same order import racing each other both found no provenance row and both inserted one, which surfaced as `duplicate key value violates unique constraint` on the import screen — a report that read like a caching anomaly and was one, from the opposite direction. The branch now answers the page with its own fetch, and names `opaqueredirect` explicitly, because a navigation POST that redirects comes back with a status of 0 and `response.ok` false on the most ordinary successful write there is. **And how hard a password has to be became the operator's decision** (§12.2). The length floor is still the default and still what the spec argues for, but it is a default rather than a law: an instance on a machine nobody else can reach spends twelve characters per sign-in defending against somebody who would already be in the room, and a toll like that is paid in passwords people can type. `PASSWORD_POLICY` names one of `12chars`, `complex`, `6chars`, `any` or `noauth`, and an unrecognized value refuses to boot rather than falling back — a misspelled security setting that starts anyway leaves the operator believing something untrue about their own instance, in whichever direction they guessed. `noauth` removes sign-in altogether and every request becomes the oldest administrator; it is announced in the log at startup, named on the health screen and bannered on every page, and it changes nothing on an instance that has no administrator yet, because turning sign-in off cannot conjure the account it would sign you in as. FR-ADM-1, FR-ADM-8, §5.6, §12.2. |
