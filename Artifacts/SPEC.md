...
- **An unknown is not priced at zero.** A service this shop has never performed has no price. It is counted, named, and left *out* of the total, and the figure is labelled a floor with its shortfall stated beside it — "3 of these have never been done here". Treating unknown as free would understate exactly the thing the report exists to warn about, which is the same mistake the cores total already refuses to make (FR-PUR-4). A total that is quietly too low is worse than no total, because it gets believed.
...
| ID | Item | From | Notes |
| --- | --- | --- | --- |
| R-1 | **Community template repository** — shared schedule templates, parser profiles, fitment data, inspection checklists | OQ-2 | Needs a trust model and a network dependency, both in tension with P-1. Import/export already covers the ninety-percent case. Hosting on GitHub is the logical choice - either a new repository or a folder within this one which HomeAutoShop can check for additions and updates. |
