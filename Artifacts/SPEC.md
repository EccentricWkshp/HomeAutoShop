...
| **v0.9.1 changes** | **A delete that deleted nothing, and the four places that hid it.** An order read in and then deleted from the admin came back as a warning nobody could act on — *already imported*, *its lines have been received* — about a purchase no screen would show. `BaseModel.delete()` stamps `deleted_at` and runs no SQL DELETE, so **`on_delete` is never consulted**: a `CASCADE` child stays alive under a parent every screen hides. The lines were still there, still marked received, and reachable from nowhere, because a line is only ever listed through its order. The reader then resolved the surviving `ExternalRef` through `all_objects`, agreed the order was present, and refused — which made the document **permanently un-re-readable**. Soft delete now cascades where a parent declares it (§5.4), parent and children sharing one timestamp so a restore brings back what that delete took and not a line somebody had already dropped on its own; the reader reads `objects` like the rest of the application, and a provenance row that outlived what it named is dropped rather than left to refuse the next import (FR-PUR-1). **The trash was the other half.** It has promised thirty days since it was written and enforced nothing: `TRASH_RETENTION_DAYS` was read by one queryset method used by one screen, and no command or job had ever removed a row — so a soft delete was permanent in the direction nobody wanted, and there was no supported way to make it leave. `manage.py purge_trash` makes the window real, `/trash/` gained a **Delete permanently** button and now lists what is past that window rather than dropping it off the page into nowhere, and `manage.py purge_order` clears up after an import that went wrong — refusing, unless told twice, to delete received stock that has since moved, because that would silently change what the shop believes it has and what it believes that cost. **And every table is in the admin**, which is where somebody goes once the application's own screens have stopped mentioning a row: nineteen models had no page at all, `PurchaseLine` and `ExternalRef` among them. All of them are registered now, the soft-deleting ones behind a record-state filter with restore and permanent-delete actions, and a single **Trash** page spans every table — because a per-model filter answers *is this row deleted?*, and the question somebody clearing up actually has is *what is in the trash?* FR-ADM-7, FR-PUR-1, §5.4. |
