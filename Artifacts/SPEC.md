...
| **v0.7.1 changes** | **Published.** Both images are on Docker Hub — amd64 and arm64 as one manifest each, built natively rather than under emulation. Three things had to be true first and none of them were. **The version was nowhere**: it lived in this document and in no file, so nothing could name an image tag. It is now `VERSION`, read by the package, stamped into `org.opencontainers.image.version`, and refused at release time if it disagrees with the git tag. **`.dockerignore` carried the same `*`-does-not-cross-a-`/` bug `.gitignore` has a comment about**: `Artifacts/samples/*.jpg` matched nothing one folder down, so every locally built image held 349 MB of untracked research and five photographs of our own vehicles. Mirroring the two lists took the image from 1.23 GB to 805 MB. **And there was no CI.** Now there is, and it runs the suite *inside the image it publishes* with the checkout mounted over `/app` — the only arrangement that tests the dependency set that actually ships, and one that immediately found the in-container suite had never been green, because `tests_locales` reads the `.po` sources the image deliberately omits. The one gate still missing is the restore drill; §15.2 says why. |
