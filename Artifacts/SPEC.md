...
| **v0.8.0 changes** | **The Vehicles screen became a board somebody arranges.** It listed vehicles alphabetically in identical cards, which answers “list my vehicles” and not the question it is actually opened for — “which of these is the truck”. Order, colour and pinned facts are now the reader's own (FR-VEH-14), per user rather than per shop, because recognition is a question two people in the same household answer differently; the dashboard's Fleet panel is the same board and no longer re-sorts itself every time anybody records an odometer reading. Rearranging works from ↑/↓ buttons with no script, for the reasons FR-WO-12 already gave, and dragging is an enhancement layered over them (FR-VEH-15). **The drag was written twice.** The first version reordered the DOM on every pointer move and chose its target by measuring the cards live, which is not a rough edge but a loop: the move reflows the grid, the reflow puts a different card under the pointer, and the board oscillates between two arrangements with the hand perfectly still. The geometry is frozen at the moment the drag begins and the preview drawn with transforms, so nothing reflows until the drop. **And that fix could not be seen, which is what found the real bug.** `offline.js` registered the worker at `/static/sw.js` rather than the `/sw.js` route built to give it the site as its scope — so it controlled a directory of stylesheets: no page was ever cached, the offline landing was unreachable, and Background Sync woke a worker watching nothing. What it did still intercept was `/static/`, cache-first, against a literal `VERSION = "v1"` that `activate` therefore never invalidated. **Every release of the stylesheet and of every script since the worker first installed was invisible to anybody who had used the app before**, and looked exactly like a change that had not deployed. **A third came in from the running instance**: a `backup.run` marked running for eighteen hours. `claim` stamps `locked_at` and nothing ever read it, so a job whose worker died — a redeploy, an OOM, the host going down — stayed `running` for ever. `schedule.tick` skips a type that already has a row in flight, so that one orphaned job **silently ended every future backup**, and the screen disabled “Back up now” for the same reason: the instance looked busy rather than broken and quietly stopped protecting itself. The worker now reclaims a stalled job at the head of every drain, counting the attempt so a job that kills its worker every time can still reach `failed` rather than looping; the backups screen names one and offers to give up on it. §19. |
...
- **Vehicles — the board.** A list of cards the reader arranges: their own order, an optional colour, and per-vehicle pins choosing what each card says (FR-VEH-14). Rearranged with ↑/↓ buttons that need no script, or by dragging where a pointer allows it (FR-VEH-15). The dashboard's Fleet panel is the same board, showing its first few.
