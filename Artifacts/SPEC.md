...
| **v0.9.6 changes** | **eBay orders can be read.** The fourth reader and the second marketplace, and it differs from the other three in ways that each change what may be believed about the document. **One payment, several orders**: eBay prints an order number per *seller*, so one checkout across three sellers is three numbered tables under one tax figure and one total — three of the eight sample orders carry more than one, so a reader that assumed one would have read a third of them and silently dropped the rest. They stay one purchase, filed under the first number, each line carrying its own seller, with the page saying so rather than quietly picking one. **The price column is the extended figure**, which nothing on a single-quantity line reveals and only the arithmetic settles: two lock nuts at $12.98 and two washers at $13.88 among nine items read as prices each come to $181.85 against a stated $154.99, and read as line totals come to it exactly — the same question the Amazon reader got wrong in the other direction. **And a refund is not a discount**, which is the one place the arithmetic could have been made to work while being wrong: a sample paid at $167.08 ends on $131.57 with $35.51 refunded for an item that never arrived, and putting that into the discount reconciles against the printed total and then spreads the credit pro-rata across all seven lines, so the six parts that did arrive each record 23% cheaper than they were. The page never says which item came back and no reader can work it out, so the order states what was paid, names the refund, and the review screen is told to leave a line out — which takes that line's own share of the tax with it and lands on the figure the page itself ends on. Each seller's table **declares its own column positions in its header** and is read against them, because a product name shares its baselines with the carrier printed eleven points to its right: in reading order `USPS Priority Mail` becomes the end of a product title and then a catalog entry under that name. The shipping address is dropped by position and never inspected — written as the column's own coordinate the boundary let it through, because a PDF library returns 189.99999999999997 for a word set at 190 (NFR-S-5). All eight sample orders reconcile against their own printed totals. FR-PUR-10, §8.3a. **A part gets its name while the order is read in, rather than around it.** A seller's title is written to be found in a search box rather than to be a name — `95526-78F00-000 Suzuki Switch,triple pressure 9552678F00000, New Genuine OEM Par (297494871661)` is one line of a real order — and filed straight in it is what that part is called for ever. The ways out were to create the part by hand *before* importing so the order matched it, or to correct it afterwards on the part's own page: the same work, done twice, at the wrong moment. The name is asked for on the line now, defaulting to what the document said, and it does **two** jobs with one set of keystrokes — it names a part that is created, and it is what the line is **looked up by** where the document states no part number, so a title nothing will ever match stops landing a second copy of something already on the shelf. It never renames a part that already exists, which is somebody's own decision about their own catalog, and the purchase keeps the document's own wording either way. The screen therefore submits to **itself** as well as forward: a name decides between finding and creating, and there was no way to see which but to commit. Every choice on it survives that round trip — the treatment selector had hard-coded its first option as selected, which cost nothing while the only button there was the one that writes — and the button that looks again is first, so Enter in a name field looks rather than writes. FR-PUR-1. **And four things found by doing it.** The review screen printed the order's whole discount against **every** named credit, which is the same number only while there is one of them. Every line left out or recorded as tooling was labeled **inside a kit**, because the pill asked whether the line was charged and a kit component is not the only uncharged thing on that screen — so it sat beside a pill already saying the line was left out. `Part.name` holds 200 characters and **was never truncated** against a general retailer's title of 400, unnoticed because the two readers that state a part number also print short descriptions; on a database that enforces its own column widths that is an import that dies rather than a name that is merely long. And a name box holding nothing but spaces produced a part with no name at all, because the value was stripped after being chosen between rather than before. |
...
| FR-PUR-10 | **A supplier order document is recognized, not declared, and a general retailer is not a parts vendor.** RockAuto was the only reader, so the shape it returned lived inside it and the import service named RockAuto in six places; two more vendors make that the wrong arrangement, because what differs between suppliers is the *document* and nothing else. The shape is `orders.py`, the vendor identifies itself on the order it returns, and the file is offered to each reader in turn — a reader recognizes its own format or **raises**, because one that tried hardest to extract something would be wrong in the expensive direction. **NAPA** prints the extended figure per line and a list price beside it, so it is taken directly (FR-PUR-9); its summary is also the evidence for the discount-before-tax rule, since 8.4% of its $228.05 subtotal is $19.16 and it is 8.4% of $223.05 that gives the $18.74 it printed. It states a pickup date, which is what a return window runs from (FR-PUR-5). **Amazon** is not a parts vendor at all: one sample basket holds eight items of which exactly one is a part and seven are tools, and nothing in the document says which — so the reader takes no view and **the review screen asks**, one choice per line: a part, the tooling it was, or nothing. **Tooling here is a spend and never an inventory** — NG-8 and NG-9 put tool and toolbox tracking permanently out of scope and WrenchLedger is where it lives, while OQ-4 keeps `expense.category = tooling` first-class, always exported and excluded from per-vehicle cost. So such a line becomes one `expense` row carrying the amount, the vendor and the date, with **no part, no `ShopTool`, no location, no serial and no model**, and with `asset` null rather than merely excluded by a setting: an expense attached to no vehicle cannot be pulled into one by a later change of mind about `COST_INCLUDE_TOOLING`. Whatever a line becomes it takes its share of tax and shipping with it, allocated by the same largest-remainder split a kit's cost uses, so the order still adds up to what it cost and nothing is banked twice. What is left out is left out of the money: tax and shipping are apportioned to the lines kept, because a purchase claiming $32.72 of tax on $14.24 of relays is a cost history that reads plausibly and is wrong for ever (G-4). Amazon prints a price *each* where NAPA prints a total, which nothing but **reconciling against the document's own figures** revealed — read the other way, a two-parcel order of washer fluid reported $11.94 of $35.82. A marketplace seller is recorded as the seller and never as the brand. **And the review screen is where a part gets its name**, because a seller's title is written to be found in a search box rather than to be a name — `95526-78F00-000 Suzuki Switch,triple pressure 9552678F00000, New Genuine OEM Par (297494871661)` is one line of a real order, and filed straight in it is what that part is called for ever. The ways out were to create the part by hand *before* importing so the order matched it, or to correct it afterwards on the part's own page: the same work, done twice, at the wrong moment. So the name is asked for on the line, defaulting to what the document said, and it does **two** jobs with one set of keystrokes — it names a part that is created, and it is what the line is **looked up by** where the document states no part number, so a title nothing will ever match stops landing a second copy of something already on the shelf. It never renames a part that already exists, which is somebody's own decision about their own catalog, and `description_as_ordered` keeps the document's words either way. The screen therefore submits to **itself** as well as forward: a name decides between finding and creating, and there was no way to see which but to commit. Every choice on it survives that round trip, and the button that looks again is the first one so that Enter in a name field looks rather than writes. **eBay** is one payment over several *orders*: it prints an order number per seller, so one checkout across three sellers is three numbered tables under one tax figure and one total, and the per-seller figures needed to split them are printed nowhere — it stays one purchase, filed under the first number, with each line carrying its own seller and the page saying so rather than quietly picking one. Its price column is the **extended** figure, which only the arithmetic reveals: two lock nuts at $12.98 and two washers at $13.88 among nine items read as prices each come to $181.85 against a stated $154.99, and read as totals come to it exactly. And **a refund is not a discount** — the one place the arithmetic could have been made to work while being wrong. A sample paid at $167.08 ends on $131.57 with $35.51 refunded for an item that never arrived; putting that into the discount reconciles against the printed total and then spreads the credit pro-rata across all seven lines, so six parts that did arrive each record 23% cheaper than they were. The page does not say which item came back and no reader can work it out, so the order states what was paid, names the refund, and the review screen is told to leave a line out — which takes that line's own share of the tax with it and lands on the figure the page itself ends on. Its columns are **read from the table header rather than assumed**, because each seller's table sets them in different places and a product name shares its baselines with the carrier printed eleven points to its right: in reading order `USPS Priority Mail` becomes the end of a product title and then a catalog entry. The shipping address is dropped by position and never inspected — written as the column's own coordinate the boundary let it through, because a PDF library returns 189.99999999999997 for a word set at 190 (NFR-S-5). §8.3a. |
...
| --- | --- |
| 0.9.6 | **eBay orders can be read** — the fourth reader and the second marketplace. It prints an order number per seller, so one checkout across three sellers is three numbered tables under one tax figure and one total; they stay one purchase, with each line carrying its own seller. Its price column is the extended figure, which nothing but the arithmetic settles, and **a refund is not a discount**: spreading a $35.51 credit for an undelivered item across the order's seven lines would record the six that did arrive as 23% cheaper than they were, so the order states what was paid, names the refund, and the screen is told to leave a line out — which lands on the figure the page itself ends on. Columns are read from each table's own header, because a product name shares its baselines with the carrier printed eleven points to its right. The shipping address is dropped by position and never read. All eight sample orders reconcile against their own printed totals. **And a part gets its name while the order is read in**, rather than by creating it by hand beforehand or correcting it afterwards: the same box that names a new part is what the line is looked up by, so a seller's title stops landing a second copy of something already on the shelf, and the review screen can be looked at again to see which happened. Found on the way — a credit row printing the order's whole discount, every left-out line labeled *inside a kit*, a `Part.name` never truncated against a 400-character title, and a name of spaces becoming a part with no name. FR-PUR-1, FR-PUR-10, §8.3a. |
