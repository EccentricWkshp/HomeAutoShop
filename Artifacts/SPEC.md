...
| **v0.9.0 changes** | **Two fields the catalog was already storing and could not show, and the arithmetic underneath a receipt.** `is_consumable` reached one line of chooser ranking and no screen; `category` reached the search box and no screen at all — so it filed nothing, and nobody could see that the last part went in under a different spelling of it. Both are now visible, filterable and counted (FR-PART-8), and a part is in **as many categories as it is actually in**: a headlight is electrical and it is lighting, and one field made somebody choose and then lose it from the direction they did not choose. Categories became rows with a case-insensitive unique constraint (FR-PART-9), because the convergence they exist for **cannot be a helper somebody remembers to call** — that was the previous shape and the CSV importer walked straight past it, in the one place a catalog arrives hundreds of rows at a time. A constraint cannot be walked past. Parts reached the API for the first time, through the *same* `matching()` and `KINDS` the screen uses (§10). **The importer stopped discarding six columns it asked for**: `unit`, `quantity`, `location` and `cost` on a parts file and `cost` and `vendor` on a service file were mapped on screen and thrown away, so a spreadsheet that knew the shop owned nine oil filters in Cabinet B produced nine filters' worth of nothing (FR-ADM-6). **Then a NAPA receipt disagreed with the screen and both halves of why were bugs.** The discount was being taken off *after* tax, so tax was charged on five dollars nobody paid — invisible until then because with tax stated as an amount, addition is commutative and a model that was wrong about *what tax is charged on* still produced a right-looking total. And a line stored the price of **one**, which cannot represent a real purchase: five gallons sold for $182.39 is $36.478 a gallon, not a number of cents, so the line claimed $182.40 and a penny appeared out of the arithmetic. Both are category errors rather than rounding ones — **a rate is not an amount anybody paid** — so a purchase carries an optional `tax_rate` worked out on the discounted subtotal, and a line stores what it cost with the per-unit figure derived (FR-PUR-1, FR-PUR-11). The vendor's own document is the evidence: NAPA prints $228.05, $18.74 of tax and a $5.00 discount against $241.79, and 8.4% of $228.05 is $19.16. **Two more order readers** (FR-PUR-10): documents are recognized rather than declared, and Amazon is not a parts vendor — one sample basket holds eight items of which exactly one is a part — so the review screen asks per line, and tooling is recorded as **a spend and never an inventory**, which is the NG-8/OQ-4 line. **And an order's own count is not the shop's** (FR-PUR-12): a two-pack of relays is one line item, one charge and two relays, and the importer was copying the vendor's line count into a field that has meant *how many of the part* to receiving, to readiness and to the hand-typed form all along — so one relay went on the shelf at twice its price and the other went nowhere. The review screen asks, the money does not move, and what the seller wrote about the pack is quoted rather than counted, because a pattern that counted it was wrong in three different ways across three sample orders. **And a page never predates the reader's own edit**: pages were stale-while-revalidate, so the screen that came back after saving was a copy taken before the save, reported as *it looks like nothing was saved*. §19. |
