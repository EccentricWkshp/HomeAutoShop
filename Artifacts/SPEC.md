...
| **v0.7.2 changes** | **A fresh machine found everything the documentation assumed.** Compose now **pulls the published images** rather than building them, which is what makes publishing them worth anything; `docker-compose.build.yml` puts the build back for the three reasons anyone still needs it — their own changes, an OCR language the image lacks, a DNS provider that is not compiled in. Walking INSTALL.md on a clean Ubuntu box then produced roughly a failure per step, and every one was a gap rather than a mistake. **§1 named a Debian packaging quirk without ever saying how to install Docker**, and had no macOS path at all; it is now three complete platform routes, including the steps other guides bury — enabling WSL 2, picking the right Mac build, `systemctl enable --now docker`, the `docker` group, and checking nothing already holds ports 80 and 443. Those ports were assumed free and are now `HTTP_PORT`/`HTTPS_PORT`, with the http→https redirect told which port to name and a startup warning when `BASE_URL` disagrees — because `BASE_URL` is what **printed QR labels** are built from, and a wrong port there is discovered by somebody scanning a bin months later. **And restoring onto another machine was not documented at all, because it did not work**: `backup_download` hands out the database file alone, the export comes down as its own ZIP, and `manifest.json` lives in the folder — so the two artifacts the Backup screen gives you are exactly the two `restore` refuses. Settings → Backup now takes them back and rebuilds the folder; restore stays a command, for the reason it always was. §7 documents the whole path, uid 10001 included. Instance health says **which machine it is running on** — version, build revision, architecture — and lists the job queue instead of counting it, since a count cannot tell three thumbnails from a backup that has been retrying for an hour. |
