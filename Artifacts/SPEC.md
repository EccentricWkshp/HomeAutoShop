...
| **v0.9.4 changes** | **A general retailer's order could not stop making new parts.** The matcher looked up a brand and a part number, and a retailer prints neither — an Amazon line has a product title and a marketplace seller and nothing else — so every such line created a part, unconditionally. That is wrong inside a *single order*: one that ships in two boxes lists the same item on a line for each, and the shop ends up holding the product twice, with two catalog rows, the stock split between them and neither able to say what is on the shelf. Three gallons and three gallons of the same washer fluid, filed apart. A line carrying no number now matches on an **exact, case-insensitive match of the whole title**, which is as conservative as this can be while still working: a retailer's title is long and specific, and two genuinely different products do not carry byte-identical ones. Nothing here is fuzzy on purpose — a near-match would eventually fold two sizes of one product together, and stock merged into the wrong row is not undoable by looking at it. The parts suppliers are untouched, because this is only ever reached where the document states no number at all. **The unit selector from 0.9.2 had pushed the status column off the card**, which is the one place the review screen says *new part*: `inline-size: auto` sized the control to `fluid ounces` and the column that lost the room was the last one. **And the suite stopped inheriting the operator's `.env`.** `PASSWORD_POLICY` is pinned to the default under the test runner, alongside the network guard and the cheap hashing that are there for exactly the same reason — a suite whose result depends on the machine it runs on is not a suite. Setting `any` to try 0.9.3's feature out failed three tests on a checkout where nothing was wrong, and `noauth` was worse: it puts a middleware in front of every request that signs it in, so whole files of permission tests went on passing while testing nothing. FR-PART-2, FR-PART-3, FR-PUR-1, §12.2. **Six more, all found by using it.** Receiving a line reloaded the purchase page and put you back at the top of it — the one action somebody does repeatedly, down a delivery — so the page is a live region now; one over the whole page rather than one per card, because receiving also moves the status pill in the header and `liveform.js` replaces only the region the form was in. **Receive, take it back, receive again** — the ordinary way to correct a receipt — left the first lot behind at zero for ever: `unreceive` does not delete and must not, since the ledger is append-only and a reversal is a movement rather than an erasure, but nothing said the next receipt had to mint a second shelf row. An empty lot from the same line is refilled instead, and only one whose entire history is receipts and their reversals qualifies — a lot emptied by *consumption* is a real cost record that FIFO reads through. **A fitment can say Universal**, as a fact rather than as a spelling: it had to be typed into the make field, which made a property of the part into a string, and a string is not a rule — `matches()` compared it against `asset.make` and the universal fitment matched no vehicle at all. **A part carries a supplier link**, typed rather than derived, because a URL cannot be built from a manufacturer and a number — RockAuto keys on its own catalog ids and Amazon has no addressable notion of *this part* — and a generated link is a guess that 404s at the moment somebody needs it. **A use recorded without a job can be corrected**: the vehicle and the date, which are the two the screen asks for and the two people get wrong. The quantity deliberately cannot be, because it is the visible half of a stock movement and editing it here would move nothing; the lot count is the honest path, and the page says so. FR-INV-1, FR-INV-5, FR-INV-10, FR-PART-3, FR-PART-4, FR-PUR-1, §9.2. **And a core deposit turned out to have three endings rather than two.** `core_returned` was a boolean, so the only way to stop a core nagging from the Owed list was to say it went back — a false statement about a part still on the shelf. The reported case is a caliper whose return shipping came to as much as the deposit, which makes keeping it the cheaper answer: the money is spent *on purpose*. That distinction is the whole point, because money written off deliberately is a decision somebody made and can defend, while money written off by forgetting is precisely what FR-PUR-4 exists to prevent — and one boolean could not tell them apart, so the list either nagged for ever or was lied to. `core_state` is owed, returned or kept; `core_returned_on` became `core_settled_on`, since a kept core has a date too and the old name was a wrong word on half the rows. **A part can carry photographs** (FR-DOC-2), which vehicles, jobs and receipts always could and the thing on the shelf could not. A part number identifies an item to a supplier and describes it to nobody: two sway bar links with adjacent numbers differ by which way the stud faces, and a bracket is the one that fits or the one that does not by a bend you can see and cannot read. Through the same `ingest` every other attachment uses, so the same photograph uploaded twice is stored once and the second upload links the copy already on file (FR-DOC-6). **And a pass over what the screen actually says.** *The tooltip on most fields just says "Please fill out this field"* — it did, because that is the browser's answer for a required control carrying no description of its own, and it is a true sentence about every empty box and a useful one about none. Every field now carries a sentence saying what it is or does, in `title`, which the browser offers on hover, a screen reader offers as the description, and `forms.js` hands to the bubble in place of that message. `title` rather than help text, because a paragraph under all one hundred and ninety fields would bury the handful that are genuinely surprising. **Only the missing-value message is replaced** — out of range, wrong format and too long already name the problem — and nothing on that path validates anything. Two gates hold it: every field of every form found by walking the package rather than from a list, and every hand-written control carrying `required`. **A fitment gained a fifth confidence**, reported as: an A/C system flush, brake cleaner, a rag or a hose clamp does not really fit confirmed, does not fit, stated by vendor, or unverified. It does not — all four are statements about evidence, and there is none to have about a bottle of cleaner, so `unverified` was leaving a permanent question mark against something nobody will ever check. `general_purpose` says it fits anything by what it is, and choosing it sets `is_universal` rather than asking twice. **And the stock lots card stopped breaking its own figures**: `.mono` breaks anywhere on purpose because a VIN has to, which set a quantity of `1.000` as `1.00` above a stray `0` — a number nobody typed, rendered perfectly. `anywhere` also drives min-content sizing, so the table crushed itself instead of scrolling; figures opt out by name now, one rule shared with the order review screen that had already met this as `$182` above `.39`. The count controls in that column are two deliberate lines rather than four boxes left to wrap, which had put *Adjust* under the reason box. FR-INV-7, FR-PART-6, §9.2. |
