...
Then open <http://localhost:8000>. Jobs run out of process:
...
The account step is separate because the database is a volume, not part of the
image: recreate the volume and you have a working instance with nobody in it.
...
```bash
python manage.py makemessages -l en_US -l en_CA -l fr_CA -l es_MX --ignore=venv --ignore=Artifacts
python manage.py compilemessages
```

The image installs gettext, so `docker compose exec app python manage.py makemessages ...`
works without setting anything up locally. Until a catalog exists, Django
falls back to the source strings, so `en-US` is complete and the other three
locales render English — which is the intended starting state.
